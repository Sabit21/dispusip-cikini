...
*/
	...
	function GetTables($table_name = '')
	{
		...
		if ($this->isOpen && isset($this->connectionId) && is_resource($this->connectionId)){
			...
			$results = mysqli_query($koneksi,$sql, $this->connectionId) or $this->HandleException();
	...
	function GetColumnsOfTable($TableName)
	{
		...
		$result = mysqli_query($koneksi,$query) or $this->HandleException();
	...
	function ExecuteSQL($aStatement, $MaxRows)
	{
		...
		$result = mysqli_query($koneksi,$aStatement) or $this->HandleException();
